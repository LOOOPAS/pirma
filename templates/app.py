...
def login():
    ...
    if request.form['submit'] == 'login':
        ...
        if user is not None and user.password == password and user.name != 'skirma':
            ...
            return render_template('main.html', table=bookdata.to_html(), table1=operdata.to_html(), table2 = finishdata.to_html())
...
def admin_action():
    ...
    if request.form.get('buy_check'):
        if position is not None:
            ...
            position.save_to_db()
            operation = OperationModel(name, amount, price, buy)
            operation.save_to_db()
            return render_template("admin.html")
        ...
        if position is not None:
            if position.amount - amount != 0:
                ...
                operation = OperationModel(name, amount, price, buy)
                operation.save_to_db()
                finish = FinishedBookModel(name, amount, price)
