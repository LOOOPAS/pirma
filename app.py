...
def login():
    ...
    if request.form['submit'] == 'login':
        ...
        if user is not None and user.password == password and user.name != 'skirma':
            ...
            bookdata = pd.read_sql('book', os.environ.get('DATABASE_URL'))
...
def pasirinkimas():
    if request.form['submit'] == 'operations':
        operdata = pd.read_sql('operations', os.environ.get('DATABASE_URL'))
        ...
        finishdata = pd.read_sql('finishedbook', os.environ.get('DATABASE_URL'))
